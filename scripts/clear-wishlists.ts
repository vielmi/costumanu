...
async function main() {
  ...
  if (!user) {
    console.error(`User ${USER_EMAIL} nicht gefunden`);
    process.exit(1);
  }
  ...
  const { data: lists } = await sb.from("wishlists").select("id").eq("owner_id", user.id);
  ...
  const { error: itemsErr } = await sb.from("wishlist_items").delete().in("wishlist_id", ids);
  if (itemsErr) console.error("wishlist_items:", itemsErr.message);
  else console.log("✓ wishlist_items gelöscht");

  const { error: listsErr } = await sb.from("wishlists").delete().in("id", ids);
